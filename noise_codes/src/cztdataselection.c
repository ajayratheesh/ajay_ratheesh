...
void processDataSelection(char *mkffile,char *thresholdfile,char *evtfile);
void writeEvent(double *evttime,double *cztseccnt,unsigned short *cztntick,unsigned short *pha,unsigned char *detid,unsigned char *pixid,unsigned char*detx,unsigned char *dety,unsigned short *veto,unsigned char *alpha,int *pi,float *energy,char *outputfile,long firstindex,long lastindex,long bufsize,int hdunum);
void printerror( int status);
void createEventFile(char *outputfile,char *evtfile,int flag);
double SAA_TIME_THRESHOLD=700.0;
double SAA_TIME_THRESHOLD_2=700.0;
float SAA_ENERGY_THRESHOLD=100.0;
...
void processDataSelection(char *mkffile,char *thresholdfile,char *evtfile)
{
	...
	int quadstart=0,quadend=3,total_quad=4;
	int isfilecreated=0,dimen=64;
	char *outfile,*outtxtfile;
	double thresh_1s;
	float AVG_THRESHOLD;
	...
	outfile=calloc(1000,sizeof(char));
	...
	char *outevtfile = strtok(evtfile, "."); 
	sprintf(outtxtfile, "%s_ds.log",outevtfile);
	remove(outtxtfile);
	logfile=fopen(outtxtfile,"a"); 

	fprintf(logfile,"Inputs for dataselection\n1.MKF file : %s\n2.Threshold file : %s\n3.Event file : %s\n\n",mkffile,thresholdfile,tempevt);
	...
	//calculate SAAshift1 and SAAshift2
		...
		SAAshift2=SAASlopeRight*(lat[ii]-SAALatRef);
			//printf("%f %f %f %f %d %d %f %f\n",lat[ii],lon[ii],SAAshift1,SAAshift2,SAALongMin,SAALongMax,SAASlopeLeft,SAASlopeRight);	
        ...
        if(nsaa==0)
	{
		...
		sprintf(outfile, "%s_0.evt",outevtfile);
		remove(outfile);
	        createEventFile(outfile,tempevt,0);	
	}
	else
	{
	...
	status=0;
	if ( fits_open_file(&evtfptr, tempevt, READONLY, &status) )
	{
	    	printf("Error in opening a file : %d",status);
	   	printerror( status );
	}   
	...
	for(i=quadstart; i<=quadend; i++)
	{
		...
		firstindex_array  = calloc(nsaa+1,sizeof(long));
		lastindex_array  = calloc(nsaa+1,sizeof(long));
	...
	for(i=quadstart; i<=quadend; i++)
	{
		...
		fits_read_col(evtfptr, TUSHORT, cztntickcolnum, frow, felem, evtnrows, &intnull, cztntick,&anynull, &status); 
		fits_read_col(evtfptr, TUSHORT, phacolnum, frow, felem, evtnrows, &intnull, pha, &anynull, &status);
		...
		for(k=0;k<evtnrows;k++)
		{
			...
			{		
				if(isfilecreated==0)
				{
					sprintf(outfile, "%s_%d.evt",outevtfile,ii);
					if(i==0)
					{	
						printf("Creating event file %s\n",outfile);					
						remove(outfile);		
						saaflag[ii]=1;
						createEventFile(outfile,tempevt,1);													
					}						
					isfilecreated=1;	
			...
			else if(evttime[k]>final_saaendtime[ii])
			{
				...
				if(ii==(nsaa-1))
				{
					sprintf(outfile, "%s_%d.evt",outevtfile,ii+1);
					if(i==0)
					{						
						remove(outfile);
						saaflag[ii+1]=1;
		...
		for(ii=0;ii<(nsaa+1);ii++)
		{
				...
				if(lastindex_array[ii]-firstindex_array[ii]>0 && saaflag[ii])
					writeEvent(evttime,cztseccnt,cztntick,pha,detid,pixid,detx,dety,veto,alpha,pi,energy,outfile,firstindex_array[ii],lastindex_array[ii],lastindex_array[ii]-firstindex_array[ii],i+2);
				else
...
//creating event file 
void createEventFile(char *outputfile,char *eventfile,int flag)
{
	...
	double timedel,telapse,exposure[4096],doublenull;
...
//write event file
void writeEvent(double *evttime,double *cztseccnt,unsigned short *cztntick,unsigned short *pha,unsigned char *detid,unsigned char *pixid,unsigned char*detx,unsigned char *dety,unsigned short *veto,unsigned char *alpha,int *pi,float *energy,char *outputfile,long firstindex,long lastindex,long writesize,int hdunum)
{
	fitsfile *fptrOut;       
