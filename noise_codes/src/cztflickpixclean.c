...
int dimen=64;
void processFlickPixReduction(char *evtfile,char *badpixfile);
void printerror( int status);
float * calculateMean(int **dph,int pix_flag_2d[dimen][dimen]);
float * calculateSD(int **dph,float *mean,int pix_flag_2d[dimen][dimen]);
...
//int pixThresholdFix(int countrate);
...
void processFlickPixReduction(char *evtfile,char *badpixfile)
{
	...
	FILE *logfile;      
	int ii,flag,status, hdunum, hdutype,  nfound, anynull,i,intnull,badpixdetidcolnum,badpixpixidcolnum,pixflagcolnum,size=1;
	...
	int **dph;
	float *sd,*mean;
	char outevtfile[1000],outbadpixfile[1000],outbtifile[1000],outlogfile[1000];
	int DEAD_PIX_THRESHOLD,DEAD,NOISY,THRESHOLD;
	...
	status=0;
	if ( fits_open_file(&badpixfptr, badpixfile, READONLY, &status) )
	{
	    	printf("Error in opening a file : %d",status);
	   	printerror( status );
	}
	...
	char *file1 = strtok(badpixfile, ".");
	sprintf(outevtfile, "%s_fc.evt",file);
	sprintf(outbadpixfile, "%s_fc.fits",file1);
	sprintf(outbtifile, "%s_fc.bti",file);
	remove(outevtfile);
    ...
    //printf("--------------TEMPEVT-----%s",evtfile);
	//createEventFile(outevtfile,evtfile);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for(bintime=evttime[0];bintime< evttime[evtnrows-1];bintime+=1.0)
		{
			...
			for(j=tempind1;j<evtnrows;j++)
			{
				...
				if(evttime[j]-bintime >= 10.0){break;}
			}
			...
			for(j=0;j<4096;j++)
			{
				...
				if(flick_detpixid_1s[j]>flick_thresh_1s)
				{
					
					pixel_badtime[qid][j] = pixel_badtime[qid][j]+1.0; 
					//flick_pix_time_1s[flick_pix_counter_1s]=bintime;
					flick_pix_1s[flick_pix_counter_1s]=j;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for(bintime=evttime[0];bintime<evttime[evtnrows-1];bintime+=10.0)
		{
			...
			for(j=tempind1;j<evtnrows;j++)
			{
				...
				if(evttime[j]-bintime >= 100.0){break;}
			}
			...
			for(j=0;j<4096;j++)
			{
				if(flick_detpixid_10s[j]>flick_thresh_10s)
				{
					
					pixel_badtime[qid][j] = pixel_badtime[qid][j]+10.0;
					flick_pix_time_10s[flick_pix_counter_10s]=bintime;
					flick_pix_10s[flick_pix_counter_10s]=j;
			...
			for(k=0;k<flick_pix_counter_10s;k++)
			{
				for(j=0;j<evtnrows;j++)
				{
					...
					if(evttime[j]-flick_pix_time_10s[k] >= 100.0){break;}
				}
...
{
    fitsfile *fptrOut;      
    int status, ii, jj,hdunum=2,i,j=0,xstart,xend,ystart,yend;
    long  fpixel, nelements,nrows;
    short badpix[128*128];
    unsigned char *pix_flag,*detx,*dety,pix_flag_2d[64][64];
