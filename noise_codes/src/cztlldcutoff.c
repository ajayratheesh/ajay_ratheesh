...
void processpixLLDcutoff(char *input_evtfile,char *input_lldfile, char *detthreshfile, char *pixthreshfile);
...
void processpixLLDcutoff(char *input_evtfile,char *input_lldfile, char *detthreshfile, char *pixthreshfile)
{
	...
	char outevtfile[1000],outlogfile[1000];
	

	f1=fopen(detthreshfile,"rt"); 
	f2=fopen(pixthreshfile,"rt");
	...
	fits_open_file(&flld,input_lldfile,READONLY,&status);
		if(status) { printerror( status );}
	...
	sprintf(outevtfile, "%s_lldcut.evt",file);
	remove(outevtfile);
