...
int dimen=64;
long size=1;
//void processNoiseReduction(char *evtfile,char *caldbfile,char *bunchfile,char *noiseReductionThresholdFile);
void processNoiseReduction(char *evtfile,char *caldbfile,char *noiseReductionThresholdFile);
void printerror( int status);
//float * calculateMean(float **dph,int pix_flag_2d[dimen][dimen]);
//float * calculateSD(float **dph,float *mean,int pix_flag_2d[dimen][dimen]);
...
void processNoiseReduction(char *evtfile,char *caldbfile,char *noiseReductionThresholdFile)
{
	...
	FILE *logfile;     
	int ii,flag,status, hdunum, hdutype,  nfound, anynull,i,intnull,badpixdetidcolnum,badpixpixidcolnum,pixflagcolnum;
	...
	//double dph[64][64];
	float *sd,*mean;
	char outevtfile[1000],outbadpixfile[1000],outlivetimefile[1000],outlogfile[1000];
	...
	sprintf(outlogfile, "%s_nc.log",evtfile);
	remove(outlogfile);
	...
	fprintf(logfile,"Inputs for Noisy Pixel Clean \n1.Event file : %s\n2.Caldb file : %s\n3.Threshold file : %s\n\n",tempevt,caldbfile,noiseReductionThresholdFile);

	if((fp = fopen(noiseReductionThresholdFile, "r")) == NULL) {
	...
	free(label); 
	if ( fits_open_file(&caldbfptr, caldbfile, READONLY, &status) )
	{
...
*/
		...
		dph=(double**)malloc(sizeof(double*)*64);

		for(ii=0;ii<64;ii++)
		{
			dph[ii]=(double*)malloc(sizeof(double)*64);
		}
		...
		modifyExposure(outevtfile,pixel_exposure,i);	
		writegtiextension(new_gtitstart, new_gtitstop,ngtinrows,outevtfile,i+10,tstart,tstop,exposure);
		writeEvent(finalevttime,finalcztseccnt,finalcztntick,finalpha,finaldetid,finalpixid,finaldetx,finaldety,finalveto,finalalpha,finalpi,finalenergy,outevtfile,l,i+2,exposure);
...
float * calculateSD(double **dph,float *mean,int pix_flag_2d[dimen][dimen])
{
	...
	variance = malloc(2*sizeof(double));
	sd = malloc(2*sizeof(float));
	for(i=0; i<dimen; ++i)
	{
