...
void processeventseperation(char *input_evtfile);
...
void processeventseperation(char *input_evtfile)
{
	...
	char outevtfile_single[1000],outevtfile_double[1000] ;
	
	if ( fits_open_file(&fevt, input_evtfile, READONLY, &status) )
	{
	...
	sprintf(outevtfile_single, "%s_single.evt",file);
	remove(outevtfile_single);
	createEventFile(outevtfile_single,tempevt);
	
	sprintf(outevtfile_double, "%s_double.evt",file);
	remove(outevtfile_double);
	createEventFile(outevtfile_double,tempevt);
	...
	for(qid=0;qid<4;qid++)
	{
		...
		redund_flag = (int*)calloc(sizeof(int),evtnrows);
		...
		writeEvent(finalevttime_single,finalcztseccnt_single,finalcztntick_single,finalpha_single,finaldetid_single,finalpixid_single,finaldetx_single,finaldety_single,finalveto_single,finalalpha_single,finalpi_single,finalenergy_single,outevtfile_single,l2,qid+2);
		writeEvent(finalevttime_double,finalcztseccnt_double,finalcztntick_double,finalpha_double,finaldetid_double,finalpixid_double,finaldetx_double,finaldety_double,finalveto_double,finalalpha_double,finalpi_double,finalenergy_double,outevtfile_double,l1,qid+2);	
			
	}//end of qid
	modifyEventHeaderParams(outevtfile_single);
	modifyEventHeaderParams(outevtfile_double);
}
...
void writeEvent(double *evttime,double *cztseccnt,unsigned short *cztntick,unsigned short *pha,unsigned char *detid,unsigned char *pixid,unsigned char*detx,unsigned char *dety,unsigned short *veto,unsigned char *alpha,int *pi,float *energy,char *outputfile,int writesize,int hdunum)
{
	...
	int tstarti,tstopi;
	double doublenull,tstart,tstop,tstartf,tstopf,exposure;
	status=0;
