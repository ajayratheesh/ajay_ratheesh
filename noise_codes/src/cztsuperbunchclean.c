...
void createLivetimeFile(char *outputfile, char *input_livfile);
void writeEvent(double *evttime,double *cztseccnt,unsigned short *cztntick,unsigned short *pha,unsigned char *detid,unsigned char *pixid,unsigned char*detx,unsigned char *dety,unsigned short *veto,unsigned char *alpha,int *pi,float *energy,char *outputfile,int bufsize,int hdunum,double exposure);
...
void processSuperBunchClean(char *input_evtfile,char *input_bunfile,char *livetimefile,char *orbitno)
{
	...
	unsigned int qid;
	int evttime_col,evtdetx_col,evtdety_col,evtPI_col,evtdetid_col,evtpixid_col,buntime_col,bunsize_col,evttime_index,size=1,buntime_df1_col,buntime_df2_col;
	...
	//AS1G06_064T01_9000001066_07759cztM0_level2_nc.evt
	char outtxtfile[1000];
	char outevtfile[1000],outlivetimefile[1000],outbtifile[1000];;
	...
	createLivetimeFile(outlivetimefile,livetimefile);
	...
	{   
	   ...
	   if(bunnrows==0)
	   {
		...
		bunsize_real = bunsize=(int*)realloc(bunsize_real,sizeof(int)*bunnrows);
		...
		*/
		...
		fits_read_col(fevt, TDOUBLE, 1, frow, felem, qgtinrows, &doublenull, gtitstart,&anynull, &status);
		fits_read_col(fevt, TDOUBLE, 2, frow, felem, qgtinrows, &doublenull, gtitstop, &anynull, &status);  
		...
		fracexp_new=(double*)malloc(sizeof(double)*livetimenrows);
		lttime_new=(double*)malloc(sizeof(double)*livetimenrows);
		BTI_exclude_TSTART=(double*)malloc(sizeof(double)*livetimenrows);
		BTI_exclude_TSTOP=(double*)malloc(sizeof(double)*livetimenrows);
		...
		gtitstart_f=(double*)malloc(sizeof(double)*gtinrows_est);
		gtitstop_f=(double*)malloc(sizeof(double)*gtinrows_est);
		...
		GTI_saturation_TSTART=(double*)malloc(sizeof(double)*(BTI_saturation_no+1));
		GTI_saturation_TSTOP=(double*)malloc(sizeof(double)*(BTI_saturation_no+1));
		...
		gtitstart_final=(double*)malloc(sizeof(double)*gtinrows_est_final);
		gtitstop_final=(double*)malloc(sizeof(double)*gtinrows_est_final);
		...
		writegtiextension(gtitstart_final, gtitstop_final,gtinrows_final,outevtfile,qid+10,tstart,tstop,exposure);
		...
		for(i=0;i<evtnrows;i++)
		{
	        ...
	        writeEvent(finalevttime,finalcztseccnt,finalcztntick,finalpha,finaldetid,finalpixid,finaldetx,finaldety,finalveto,finalalpha,finalpi,finalenergy,outevtfile,l,qid+2,exposure);
		printf("%d end\n",qid);
...
void createLivetimeFile(char *outputfile, char *input_livfile)
{
	...
	if ( fits_open_file(&fptrlive,input_livfile, READONLY, &status) ) 
	         printerror( status );    
...
void writeLivetime(double *time,double *livetime,int livetimesize,char *outputfile,int hdunum)
{
	...
	int status, hdutype,intnull;
	long frow, felem,nrows;
	double doublenull,tstart,tstop;
	unsigned char bytenull;
	status=0;
